...
impl UiNode<'_> {
    ...
    pub(crate) fn center(&self) -> Xy<f32> {
        let rect = self.node().animated_rect;
    ...
    pub(crate) fn rect(&self) -> XyRect {
        return self.node().animated_rect * self.ui.sys.unifs.size;
    }

    pub(crate) fn render_rect(&self) -> RenderInfo {
        let size = self.ui.sys.unifs.size;
        return RenderInfo {
            rect: self.node().animated_rect.to_graphics_space_rounded(size),
...
impl UiNode<'_> {
    ...
    pub fn clicked_at(&self) -> Option<Click> {
        ...
        let node_rect = self.node().animated_rect;
