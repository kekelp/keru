...
// 
// This is the main way to make GUI code reusable, and to create "components".
// 
...
/// 
// /// 
// pub fn anon_subtree<T>(subtree_block: impl FnOnce() -> T) -> T {
...
// /// 
// pub fn subtree<T>(key: SubtreeKey, subtree_block: impl FnOnce() -> T) -> T {
...
pub fn exit_subtree(out_of_subtree_block: impl FnOnce()) {       
    if let Some(last_subtree_id) = thread_local::last_subtree() {
        ...
        log::error!("exit_subtree was called, but no subtree was entered!");
        out_of_subtree_block();
    };
}
...
impl Ui {
    pub fn anon_subtree(&mut self) -> UiSubtree {
        let subtree_id = Id(thread_local::current_tree_hash());
    ...
    pub fn subtree(&mut self, key: SubtreeKey) -> UiSubtree {
        return UiSubtree {
            key,
        };
    }
