...
pub(crate) static FAKE_TIME: AtomicU64 = AtomicU64::new(0);
...
impl<T> Observer<T> {
    pub fn is_changed(&self, last_frame_end: u64) -> bool {
        dbg!(last_frame_end, self.changed_at);
...
impl Ui {
    ...
    pub fn observe_changes<T>(&self, observer: &mut Observer<T>) -> bool {
        return observer.is_changed(self.sys.last_frame_end_fake_time);
    }
}
