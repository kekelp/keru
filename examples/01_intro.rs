...
fn update_ui(state: &mut State, ui: &mut Ui) {
    ...
    // It has `ChildrenLayout::Stack` instead of `ChildrenLayout::Free` in its `children_layout` field,
    ...
    // It's a child of the tree root, and can position itself freely in the space of the whole window.
...
fn main() {
    // The examples use the `run_example_loop` helper, which sets up a winit/wgpu loop, and runs our `update_ui` on every frame.
    ...
    // It's really not that much code, and it gives your program full control over what code gets executed and when, 
    // it allows it to access all advanced winit features, and to easily integrate custom wgpu rendering.
...
// - the `aesthetics_*.rs` examples show how the Node's styling params can be used to achieve some different aesthetics.
