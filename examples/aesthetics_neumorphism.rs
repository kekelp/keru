...
const BUTTON: Node = keru::node_library::BUTTON
    .color(BACKGROUND)
...
const BUTTON_CIRCLE: Node = BUTTON.shape(Shape::Circle);
const BUTTON_HEXAGON: Node = BUTTON.shape(Shape::Hexagon { size: 1.0, rotation: 0.0 });

fn update_ui(_state: &mut State, ui: &mut Ui) {
    let background = keru::node_library::PANEL.color(BACKGROUND).size(Size::Fill, Size::Fill);
