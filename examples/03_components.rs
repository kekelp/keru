...
/// 
/// Components are the most robust way of separating GUI code into reusable "components".
/// In addition, Components can have their own "implicit" state.
/// This is an advanced feature: most programs will be fine without it.
///
/// In this example, we'll write a reusable counter component,
/// with a configuration setting that allows adding more than 1 at a time to the count.
///
...
pub struct Counter<'a> {
    ...
    /// the user will be able to choose the component's placement and size how they want,
    /// like when adding a regular Node.
    pub layout: Layout,
    /// Optional extra customization.
    pub color: Color,
}

/// Every `Counter` instance will have a `CounterSettings` instance as its associated state.
/// It has to implement `Default`, so that the Ui can initialize the 
/// state automatically whenever an instance of the Component is added.
...
// We finally implement the Component trait
// and specify what should happen to the Ui when the user adds the component.
impl Component for Counter<'_> {
    ...
    // The main purpose is to return an `UiParent`, so that the user can add children to it:
    // `ui.add_component(counter_component).nest(|| { ... })`.
    type AddResult = ();
    // Another type that we're not using in this example.
    // See the `drag_component.rs` or `aesthetics_scifi.rs` examples. 
    type ComponentOutput = ();
    // ...hopefully future versions of Rust will let the trait use default values for these types.

    fn add_to_ui(&mut self, ui: &mut Ui, state: &mut Self::State) {
        // (Using an arena is not mandatory, but allocating a tiny String on the global heap
        // just to format a value is not a good thing.
        // (Keru has an arena that you can use without any setup,
        // and it will make small local allocations virtually free.)
        with_arena(|arena| {

            let panel = PANEL.layout(self.layout).color(self.color);

            let count_text = bumpalo::format!(in arena, "Count: {:.2}", self.count);

            // We can nest another component inside this one.
            let slider = Slider { value: &mut state.increase_step, min: 0.0, max: 10.0, clamp: true };
            ...
            // This is fine: each instance of the Component gets its own private "key scope",
            // and can treat its keys as unique.
            // You can do this manually outside of a Component by using `ui.key_scope()`.
            // See the `key_scope.rs` example.
        });
    }
}
...
/// 
/// Rather than sticking a reference inside of the component struct,
/// we could also return the delta value as the result of ui.add_component, 
...
/// ```
/// 
/// We could also add a [ComponentKey] to the component struct,
/// implement the optional [Component::component_key()] method,
/// and then return the delta from the [Component::run_component()] method.
/// This way, using the component would look very similar to adding a regular Node
/// and calling `ui.is_clicked(KEY)` on it:
/// 
/// ```
